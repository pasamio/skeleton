 ...
 * @since       13.1
 */
class QueryOracle extends QueryPdo implements QueryPreparable, QueryLimitable
{
	/**
	 * @var integer
	 * @since 13.1
	...
	public function processLimit($query, $limit, $offset = 0)
	{
		...
		if ($limit || $offset)
		{
			$query = "SELECT gg2.*
		              FROM (
		                  SELECT gg1.*, ROWNUM AS gg_db_rownum
		                  FROM (
		                      " . $query . "
		                  ) gg1
		              ) gg2";
			...
			if ($limit > 0)
			{
				$query .= ' WHERE gg2.gg_db_rownum BETWEEN ' . ($offset + 1) . ' AND ' . ($offset + $limit);
			}
			else
			{
				...
				if ($offset)
				{
					$query .= ' WHERE gg2.gg_db_rownum > ' . ($offset + 1);
				}
			}
		}
