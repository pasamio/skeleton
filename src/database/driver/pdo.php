 ...
 * @since       13.1
 */
abstract class DriverPdo extends Driver
{
	/**
	...
	public function connect()
	{
		...
		{
			case 'cubrid':
				$this->config['port'] = (isset($this->config['port'])) ? $this->config['port'] : 33000;

				$format = 'cubrid:host=#HOST#;port=#PORT#;dbname=#DBNAME#';

				$replace = array('#HOST#', '#PORT#', '#DBNAME#');
				$with = array($this->config['host'], $this->config['port'], $this->config['database']);

				break;

			case 'dblib':
				$this->config['port'] = (isset($this->config['port'])) ? $this->config['port'] : 1433;
			...
			case 'firebird':
				$this->config['port'] = (isset($this->config['port'])) ? $this->config['port'] : 3050;

				$format = 'firebird:dbname=#DBNAME#';

				$replace = array('#DBNAME#');
				$with = array($this->config['database']);

				break;

			case 'ibm':
				$this->config['port'] = (isset($this->config['port'])) ? $this->config['port'] : 56789;
			...
			case 'informix':
				$this->config['port'] = (isset($this->config['port'])) ? $this->config['port'] : 1526;
				$this->config['protocol'] = (isset($this->config['protocol'])) ? $this->config['protocol'] : 'onsoctcp';
				...
				else
				{
					...
					$with = array($this->config['host'], $this->config['port'], $this->config['database'], $this->config['server'], $this->config['protocol']);
				}
			...
			case 'oci':
				$this->config['port'] = (isset($this->config['port'])) ? $this->config['port'] : 1521;
				$this->config['charset'] = (isset($this->config['charset'])) ? $this->config['charset'] : 'AL32UTF8';
				...
				$format .= ';charset=' . $this->config['charset'];
			...
			case 'odbc':
				...
				$with = array($this->config['dsn'], $this->config['user'], $this->config['password']);

				break;

			case 'pgsql':
				$this->config['port'] = (isset($this->config['port'])) ? $this->config['port'] : 5432;
	 ...
	 *
	 * @return  DriverPdo  This object to support method chaining.
	 *
	 * @since   13.1
	 */
	public function setQuery($query, $offset = null, $limit = null, $driverOptions = array())
	{
		...
		// Store reference to the Query instance:
	...
	public function loadNextObject($class = 'stdClass')
	{
		...
		$row = $this->fetchObject(null, $class);
		if ($row)
		{
			return $row;
		}
