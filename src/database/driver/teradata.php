...
class DriverTeradata extends Driver
{
	...
	public function execute()
	{
		...
		if (!is_resource($this->connection))
		{
			Log::add(sprintf('Executing the query failed with error code `%s` and message `%s`.', $this->errorNum, $this->errorMsg), Log::ERROR, 'database');
		...
		if (!$this->cursor)
		{
			...
			if (!$this->connected())
			{
				...
				{
					Log::add(sprintf('Executing the query failed with error code `%s` and message `%s`.', $this->errorNum, $this->errorMsg), Log::ERROR, 'databasequery');
					throw new RuntimeException($this->errorMsg, $this->errorNum);
				}
	...
	{
		throw new BadMethodCallException(sprintf('%s->getCollation() not supported.', get_class($this)));
	}
	...
	public function getTableCreate($tables)
	{
		...
		{
			$query->where('DatabaseName = ' . $this->quote($this->options['database']));
		}
	...
	{
		throw new BadMethodCallException(sprintf('%s->lockTable() not supported.', get_class($this)));
	}
	...
	/**
	 * Method to select a database for use.
	 *
	 * This method is not supported for this driver.
	 *
	...
	{
		throw new BadMethodCallException(sprintf('%s->select() not supported.', get_class($this)));
	}

	/**
	 * Method to set UTF support.
	 *
	 * This method is not supported for this driver.
	 *
	 * @return  void
	...
	{
		throw new BadMethodCallException(sprintf('%s->setUTF() not supported.', get_class($this)));
	}

	/**
	 * Method to unlock any locked tables.
	 *
	 * @return  void
	 *
	 * @since   13.1
	...
	{
		throw new BadMethodCallException(sprintf('%s->unlockTables() not supported.', get_class($this)));
	}
